...
class MLP_adp(torch.nn.Module):
    def __init__(self, input_size, out_size, dropout=0.2):
        ...
        # self.mlp_4 = nn.Linear(int(input_size / 8), out_size)
...
class SemanticContrast(Module):
    def __init__(self, n_node, lr, layers, l2, alpha, beta, dataset, num_heads=4, emb_size=64, batch_size=100, num_item_negatives=100,num_sess_negatives=50):
        ...
        self.meger_id_img_txt = MLP_merger(self.emb_size*2, self.emb_size, dropout=0.5)
        ...
        emb_path_pre = './datasets/'+ dataset + '/pca100/'
        ...
        self.sess_con_mlp = torch.nn.ModuleList([MLP_conspace(self.emb_size, self.emb_size, dropout=0.5) for _ in range(4)])
        self.item_con_mlp = torch.nn.ModuleList(
            [MLP_conspace(self.emb_size, self.emb_size, dropout=0.5) for _ in range(4)])
    ...
    def generate_sess_emb(self, item_emb_final, text_emb_ori, text_emb_Swapword, text_emb_Bertsub, session_item, session_len, reversed_sess_item, mask):
        ...
        return id_seq_emb, augtxt1_seq_emb, augtxt2_seq_emb, augtxt3_seq_emb

    def fusion_img_text_gate(self, id_emb, text_emb_ori):
        id_embedding = id_emb
        merge_temp = torch.cat([id_embedding,text_emb_ori], 1)
        temp_emb = self.meger_id_img_txt(merge_temp)
        g2 = torch.tanh(self.gate_w1(temp_emb) + self.gate_w2(text_emb_ori))

        item_final_emb = id_embedding + g2 * text_emb_ori
    ...
    def item_contrastive(self, item_emb_final, text_emb_ori, text_emb_Swapword, text_emb_Bertsub):
        zeros = torch.cuda.FloatTensor(1, self.emb_size).fill_(0)
        ...
        text_emb_ori_mlp = self.item_con_mlp[1](text_emb_ori)
        text_emb_Swapword_mlp = self.item_con_mlp[2](text_emb_Swapword)
        text_emb_Bertsub_mlp = self.item_con_mlp[3](text_emb_Bertsub)
        ...
        select_view = torch.randint(3, (self.n_node,1))
        ...
        neg_aug3_emb = self.item_con_mlp[0](neg_aug3_no_emb)
        neg_aug4_emb = self.item_con_mlp[1](neg_aug4_no_emb)
        neg_aug5_emb = self.item_con_mlp[2](neg_aug5_no_emb)
    ...
    def sess_contrastive(self, sess_emb, aug1_sess_emb, aug2_sess_emb, aug3_sess_emb):
        tau = 1
        ...
        select_view = torch.randint(3, (self.batch_size, 1))
    ...
    def forward(self, session_item, session_len, reversed_sess_item, mask):
        ...
        # image& text embedding original and augmented


        text_emb_ori = self.text_emb_ori.weight
        text_emb_Swapword = self.text_emb_Swapword.weight
        # text_emb_Deleteword = self.text_emb_Deleteword.weight
        # text_emb_Bertinsert = self.text_emb_Bertinsert.weight
        text_emb_Bertsub = self.text_emb_Bertsub.weight
        # text_emb_Wordnetsub = self.text_emb_Wordnetsub.weight

        # fusion item id&image&text
        item_emb_final = self.fusion_img_text_gate(id_emb, text_emb_ori)

        # item-level contrast
        item_con_loss = self.item_contrastive(item_emb_final, text_emb_ori, text_emb_Swapword, text_emb_Bertsub)

        # sequence encoder including item_ID&augmented sequence
        sess_emb, aug1_sess_emb, aug2_sess_emb, aug3_sess_emb = self.generate_sess_emb(item_emb_final, text_emb_ori, text_emb_Swapword, text_emb_Bertsub, session_item, session_len, reversed_sess_item, mask) #batch内session embeddings
        sess_con_loss = self.sess_contrastive(sess_emb, aug1_sess_emb, aug2_sess_emb, aug3_sess_emb)
